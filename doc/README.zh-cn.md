...
如果你嫌corrigialink+OpenVPN麻烦，你可以尝试tinyfecVPN，一个集成了corrigialink功能的VPN：
...
对于windows和mac用户，在虚拟机中可以稳定使用（speeder跑在Linux里，其他应用照常跑在window里，桥接模式测试可用）。可以使用[这个](https://github.com/shei254/udp2raw-tunnel/releases/download/20171108.0/lede-17.01.2-x86_virtual_machine_image.zip)虚拟机镜像，大小只有7.5mb，免去在虚拟机里装系统的麻烦；虚拟机自带ssh server，可以scp拷贝文件，可以ssh进去，可以复制粘贴，root密码123456。
...
```
在server端运行:
./corrigialink -s -l0.0.0.0:4096 -r127.0.0.1:7777  -f20:10 -k "passwd" --mode 0

在client端运行:
./corrigialink -c -l0.0.0.0:3333 -r44.55.66.77:4096 -f20:10 -k "passwd" --mode 0
```
...
这里推荐的参数是给日常/非游戏情况下使用的；玩游戏请用 [使用经验](https://github.com/shei254/corrigialink/wiki/使用经验) 里推荐的参数。

##### 提示

对于某些运营商，corrigialink跟udp2raw配合可以达到更好的速度，udp2raw负责把UDP伪装成TCP，来绕过运营商的UDP限速。
...
具体见，https://github.com/shei254/corrigialink/wiki/mode和mtu选项
...
##### `-k`选项
指定一个字符串，server/client间所有收发的包都会被异或，改变协议特征，防止corrigialink的协议被运营商针对。

##### `--disable-obscure`
corrigialink默认情况下会对每个发出的数据包随机填充和异或一些字节(4~32字节)，这样通过抓包难以发现你发了冗余数据，防止VPS被封。这个功能只是为了小心谨慎，即使你关掉这个功能，基本上也没问题，关掉可以省一些带宽和CPU。`--disable-obscure`可以关掉这个功能。

# 推荐参数

https://github.com/shei254/corrigialink/wiki/推荐设置

# 使用经验

https://github.com/shei254/corrigialink/wiki/使用经验
